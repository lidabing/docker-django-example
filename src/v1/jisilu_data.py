...
def updata_realtime_bonds_market_data():
    ...
    if response.status_code == 200:
        ...
        redis_manager.set_data(
            "realtime_bonds_market_data", realtime_bond_market_data
        )
    else:
...
def update_upcoming_bonds():
    ...
    for bond_data in all_bonds_data:
        ...
        if extract_date_info(record_dt) != None and compare_dates(
            apply_date, get_today_date()
        ):
...
def update_upcoming_adjust_bonds():
    ...
    for adjust_bond in adjust_bonds_data:
        ...
        # 15天以内可能要下修的转债，才会记录
        adjust_count = find_property_value(
            adjust_bonds_data, bond_code, "adjust_count"
        )
        ...
        premium_rt = find_property_value(
            adjust_bonds_data, bond_code, "premium_rt"
        )
        ...
        # 剩余到期年限
        year_left = find_property_value(
            basic_bonds_data, bond_code, "year_left"
        )

        # premium_rt_str =  str(premium_rt) + '%'
        readjust_dt = find_property_value(
            adjust_bonds_data, bond_code, "readjust_dt"
        )
...
def update_upcoming_adjust_condition_bonds():
    ...
    for bond_data in adjust_bonds_data:
        ...
        if is_within_one_month(readjust_dt):
            bond_nm = find_property_value(
                adjust_bonds_data, bond_code, "bond_nm"
            )
            bond_id = find_property_value(
                adjust_bonds_data, bond_code, "bond_id"
            )
            premium_rt = find_property_value(
                adjust_bonds_data, bond_code, "premium_rt"
            )
...
def update_expired_bonds():
    ...
    for item in redeem_bonds_data:
        ...
        premium_rt = find_property_value(
            basic_bonds_data, item["bond_id"], "premium_rt"
        )
    ...
    redis_manager.set_data(
        "upcoming_mandatory_redeem_bonds", not_redeem_condition
    )
    redis_manager.set_data(
        "mandatory_redeem_condition_bonds", redeem_condition
    )
